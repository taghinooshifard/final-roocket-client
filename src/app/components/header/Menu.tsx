...
export default function Menu(params: Props) {
  ...
  return (
    <nav
      className={` ${
        params.showTopMenu &&
        `fixed top-0 z-50 bg-red-500 shadow-md md:px-20 opacity-95`
      } flex items-center justify-between w-full mx-auto text-white dark:bg-gray-700`}
    >
      <ul className="flex flex-row font-medium mt-0 space-x-8 max-sm:space-x-2 p-4 rtl:space-x-reverse text-sm ">
      ...
      <div className="relative px-4">
        {isFilterOpen() ? (
          <button
            ...
            className="flex items-center dark:text-white hover:bg-red-400 p-2 rounded-md"
          >
            <MdDeleteForever />
            Filter
          </button>
